...
class ParkingLearner:
    ...
    def update_state(self, direction_index: int, lenght_index: int) -> bool:
        ...
        lenght = self.index2dlength(lenght_index)
        ...
        self._state['phi'] = int(np.rint(np.rad2deg(phi_t)/10)) % 36
        self._state['orientation'] = int(
            np.rint(np.rad2deg(orientation_t)/10)) % 36
        return rho_t <= 60.0
    ...
    def parking(self, distance: float, angle: float, orientation: float):
        ...
        while self._parking:
            ...
            if self._action == 'explore':
                ...
                action_lenght_index = random.randint(0, 19)
                old_state = {
                    'rho': self._state['rho'],
                    'phi': self._state['phi'],
